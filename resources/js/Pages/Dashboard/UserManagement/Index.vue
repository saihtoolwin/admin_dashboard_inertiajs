<template>
    ...
    <div class="space-y-5 bg-white rounded-lg">
       <TableContainer :data-count="users?.data?.length" :paginate="users" :href="userListRoute">
        <template #actions>
                    <div class="flex flex-col-reverse  md:flex-row md:space-y-0 space-y-5 justify-between">
                        ...
                        <div class="relative">
                            <div class="w-[250px]">
                                ...
                                <!-- <InertiaLinkButton 
                                    ...
                                    :href="route('admin.users-management.create')" class="items-start !rounded-md bg-primary text-white !text-sm !px-3">
                    ...
                    <div class="overflow-x-auto w-full">
                        <Table :items="users.data">
                            ...
                            <template #table-header>
                                <SortableTableHeaderCell label="Name" sort="name" :url="userListRoute" />
                                <SortableTableHeaderCell label="Email" sort="email" :url="userListRoute" />
                            ...
                            <template #table-data="{ item }">
                                ...
                                <TableDataCell class="min-w-[150px]">{{ item.phone_number }}</TableDataCell>
                                <!-- <TableDataCell>{{ item.user_profile?.date_of_birth || '-' }}</TableDataCell> -->

                                <!-- Actions: Edit and Delete -->
                                <TableActionCell v-if="can('users.update') || can('users.delete')">
                                    <InertiaLinkButton
                                        v-if="can('users.update')"
                                        :href="route('admin.user-management.edit', { user: item?.id })" tooltip="Edit" class="bg-primary  !shadow-none !px-3 !py-2 text-white rounded-md">
                                        <i class="mdi mdi-file-edit"></i>
                                    </InertiaLinkButton>
                                    <NormalButton 
                                        v-if="can('users.delete')"
                                        @click="deleteCustomer(item)" tooltip="Delete" class="bg-red-600  !shadow-none !px-3 !py-2 text-white rounded-md">
